...
@app.command()
def audit(
    pdf: Optional[str] = typer.Option(None, "--pdf", "-f", help="Single PDF path or URL."),
    input_list: Optional[str] = typer.Option(None, "--input-list", "-i", help="Path to file with list of PDFs."),
    ...
    if not pdf and not input_list:
        typer.echo(f"{Fore.RED}❌ You must provide either a PDF path or --input-list{Style.RESET_ALL}")
